...
Run ``timemachine``::
  ...
  docker run --rm -v $PWD/example_config:/config clearcode/jira_timemachine:1.2.1 timemachine --config /config/config.json

Run ``timecheck``::

  docker run --rm -v $PWD/example_config:/config clearcode/jira_timemachine:1.2.1 timecheck --config /config/config.json
